...
export function ContactFooter() {
  return (
    <footer className="page-gutter pb-8 pt-20 lg:pt-28">
      <div className="grid w-full border border-slate-200 bg-slate-50 lg:grid-cols-[.72fr_1.28fr]">
        <div className="relative min-h-[430px] overflow-hidden border-b border-slate-200 p-8 lg:border-b-0 lg:border-r">
          <Link href="/" className="text-2xl font-extrabold tracking-tight text-blue-900">Smile<span className="text-red-500">.</span>Dental®</Link>
          <div aria-hidden className="absolute left-[42%] top-[22%] h-[430px] w-[430px] rounded-full border border-blue-900/25 before:absolute before:inset-12 before:rounded-full before:border before:border-blue-900/25 after:absolute after:inset-24 after:rounded-full after:border after:border-blue-900/25">
          ...
          <div className="absolute bottom-8 left-8 flex gap-3 text-blue-900">
            {[FaInstagram, FaLinkedinIn, FaFacebookF].map((Icon, i) => <a key={i} href="#" aria-label={["Instagram","LinkedIn","Facebook"][i]} className="grid h-10 w-10 place-items-center border border-slate-200 bg-white transition hover:bg-blue-900 hover:text-white"><Icon /></a>)}
          </div>
        </div>
        <div id="cita" className="p-6 sm:p-10 lg:p-16">
          ...
          <form className="mt-9 grid gap-4 sm:grid-cols-2">
            ...
            <label className="sm:col-span-2"><textarea className="min-h-32 w-full resize-y border border-slate-200 bg-white p-4 text-sm outline-none focus:border-blue-900" placeholder="Mensaje" aria-label="Mensaje" /></label>
            <label className="flex items-center gap-3 text-sm text-slate-500 sm:col-span-2"><input type="checkbox" className="h-5 w-5 accent-blue-900" /> Acepto los términos y la política de privacidad</label>
