  ...
  return (
    <>
      <header
        className={`${
          overlay
            ? `fixed inset-x-0 px-0 lg:px-12 top-0 transition-[background-color,color] duration-500 ${
                transparent
                  ? "bg-transparent text-white"
                  : "bg-white/90 text-slate-700 backdrop-blur-md"
              }`
            : "sticky top-0 bg-white/90 backdrop-blur-md"
        } z-50 animate-[header-reveal_1.5s_cubic-bezier(0.22,1,0.36,1)]`}
      >
        <div
          className={`${
            constrained
            ...
            >
              <div className="flex items-center pr-14">
                ...
                <button
                  type="button"
                  onClick={() => setMenuOpen(false)}
                  className="absolute top-5 right-4 grid size-12 cursor-pointer place-items-center border border-slate-300 p-3 text-2xl text-slate-950 transition-colors hover:border-blue-900 hover:text-blue-900"
                  aria-label="Cerrar menú"
                >
                  <FiX />
                </button>
